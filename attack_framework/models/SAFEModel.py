...
from models.SAFE.safe import calculate_similarity, calculate_similarity_batch, calculate_embeddings, \
    calculate_similarity_pool


class SAFEModelForQueryAttack(ModelForQueryAttack):
    ...
    def evaluate(self, is_first=True):
        if is_first:
            ...
            return calculate_similarity_pool(pool_embeddings, self.r2_source['cfg'])
        return calculate_similarity_pool(None, self.r2_source['cfg'], self.target_embd, len(self.r2_target_variants))

    def evaluate_batch(self, cfg_sources, multiproc_pool, variant_idx):
        target_embd = f"{self.target_embd}_{variant_idx}.json"

        return calculate_similarity_batch(cfg_sources, target_embd, pool=multiproc_pool,
                                          is_stat=self.is_stat, detailed_logger=self.detailed_logger)


class SAFEModelForPool(ModelForPool):
...
class SAFEModelForTransfer(ModelForTransfer):
    ...
    def calculate_similarity(self):
        ...
        return calculate_similarity(source_cfg, target_cfg, target_json=None, is_stat=False, detailed_logger=None)
